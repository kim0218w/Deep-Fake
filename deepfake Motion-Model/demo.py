...
def load_checkpoints(config_path, checkpoint_path, device):
    ...
    try:
        ...
        if not os.path.exists(checkpoint_path):
            print(f"Warning: Checkpoint file does not exist at {checkpoint_path}")
            print("Please make sure you have downloaded the pre-trained model.")
        ...
        print("Make sure the checkpoint path is correct and the file exists.")
        print("Continuing with untrained models.")
        
        # Still move models to device
        kp_detector.to(device)
        dense_motion_network.to(device)
        inpainting.to(device)
        avd_network.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cpu", dest="cpu", action="store_true", help="cpu mode.", default=True)
